...
"Usage:
   postprocess_fit <output-name> [-s <num-samples> -c <chains> --outcomes=<outcomes> --treatment-var=<treatment-var> --use-stanfit --no-rhats --summarize --hist-only]"
...
processed_data <- postprocess_model_fit(model_fit, stan_data, db_src, num_samples, verbose = TRUE, summarize_est = script_options$summarize %||% FALSE, 
                                        ...
                                        treatment_variable = unlist(str_split(script_options$`treatment-var` %||% NULL, ",")), 
                                        outcomes = outcomes) 
